...
class Discriminator(nn.Module):
  ...
  def __init__(self, D_ch=64, D_wide=True, resolution=128,
    ...
    if self.AC:
      self.linear_ac = self.which_linear(self.arch['out_channels'][-1], self.n_classes)
    if self.TAC:
      self.linear_tac = self.which_linear(self.arch['out_channels'][-1], self.n_classes)
